...
from src.tce_lite_v3 import TCELiteV3, TCELiteV3WithCorruptionTraining
...
def main():
    ...
    print("\nTCE-Lite v3: Early cross-modal connections + corruption-aware training")
    ...
    # Train TCE-Lite v3 with corruption-aware training
    print("\n" + "="*70)
    print("TCE-LITE V3 WITH CORRUPTION-AWARE TRAINING")
    print("="*70)
    tce_lite = TCELiteV3WithCorruptionTraining(image_dim=64, text_dim=32, n_classes=10,
                                                corruption_prob=0.4, noise_level=0.3).to(device)
    tce_clean_acc = train_model(tce_lite, train_loader, val_loader, device, 
                                 epochs=30, model_name="TCE-Lite-v3")
    ...
    for corruption in corruption_types:
        ...
        tce_acc = evaluate_corruption(tce_lite, val_loader, device, corruption, "TCE-Lite-v3")
        
        baseline_results.append(baseline_acc)
        tce_results.append(tce_acc)
        
        print(f"{corruption:20s}: Baseline={baseline_acc:.4f}, TCE-Lite-v3={tce_acc:.4f}")
    ...
    print(f"{'TCE-Lite-v3':<16} {tce_results[0]:<10.4f} {tce_results[1]:<12.4f} {tce_results[2]:<12.4f} {tce_results[3]:<12.4f} {tce_results[4]:<12.4f}")
    print("="*70)
    ...
    print(f"TCE-Lite v3 wins on {wins}/4 corruption conditions")
    print(f"TCE-Lite v3 loses on {losses}/4 corruption conditions")
    
    if wins > losses:
        print("\n*** TCE-Lite v3 shows improvement with early cross-modal connections! ***")
    elif wins == losses:
        print("\n*** TCE-Lite v3 is competitive but not clearly better ***")
    else:
        print("\n*** Still negative results - need further architectural changes ***")
    
    print("\nPer-condition differences:")
    for corruption, diff in improvements:
        status = "WIN" if diff > 0 else "LOSS" if diff < 0 else "TIE"
